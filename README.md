...
一组在线工具：PDF 的合并/拆分/压缩/格式转换，以及银行卡、IP、USDT 的批量查询。

目前是纯前端实现，处理在浏览器里完成，所以不需要后端、不产生服务器费用。
**但这是当前的实现方式，不是对外承诺**——站点文案里刻意不写「文件不上传」「零请求」这类话，
以后要加服务器时不至于自打脸。改动文案时请守住这条。
...
统计在 Cloudflare 侧完成，**页面里一行脚本都不用加**，也不影响加载速度。
